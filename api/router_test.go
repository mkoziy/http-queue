...
func TestPathParam_NormalExtraction(t *testing.T) {
	got := pathParam("/queues/myqueue/jobs", "/queues/", "/jobs")
	if got != "myqueue" {
		t.Errorf("pathParam = %q, want %q", got, "myqueue")
	}
}

func TestPathParam_NoSuffix(t *testing.T) {
	got := pathParam("/workers/myid", "/workers/", "")
	if got != "myid" {
		t.Errorf("pathParam = %q, want %q", got, "myid")
	}
}

func TestPathParam_EmptyPath(t *testing.T) {
	got := pathParam("", "/prefix/", "/suffix")
	if got != "" {
		t.Errorf("pathParam = %q, want %q", got, "")
	}
}

func TestPathParam_NonMatchingPrefix(t *testing.T) {
	got := pathParam("/other/prefix/value", "/queues/", "/jobs")
	if got != "" {
		t.Errorf("pathParam = %q, want %q", got, "")
	}
}

func TestPathParam_NonMatchingSuffix(t *testing.T) {
	got := pathParam("/queues/myqueue/other", "/queues/", "/jobs")
	if got != "" {
		t.Errorf("pathParam = %q, want %q", got, "")
	}
}

func TestPathParam_PathEqualsPrefix(t *testing.T) {
	got := pathParam("/queues/", "/queues/", "/jobs")
	if got != "" {
		t.Errorf("pathParam = %q, want %q (empty because suffix not found)", got, "")
	}
}

func TestPathParam_EmptyPrefix(t *testing.T) {
	got := pathParam("/queues/myqueue/jobs", "", "/jobs")
	if got != "/queues/myqueue" {
		t.Errorf("pathParam = %q, want %q", got, "/queues/myqueue")
	}
}

func TestPathParam_EmptySuffix(t *testing.T) {
	got := pathParam("/queues/myqueue/jobs", "/queues/", "")
	if got != "myqueue/jobs" {
		t.Errorf("pathParam = %q, want %q", got, "myqueue/jobs")
	}
}

func TestPathParam_WithNestedPath(t *testing.T) {
	got := pathParam("/queues/deep/queue/jobs", "/queues/", "/jobs")
	if got != "deep/queue" {
		t.Errorf("pathParam = %q, want %q", got, "deep/queue")
	}
}

func TestPathParam_ExtractsJobID(t *testing.T) {
	got := pathParam("/jobs/01AR3Z00000000000000000000/ack", "/jobs/", "/ack")
	if got != "01AR3Z00000000000000000000" {
		t.Errorf("pathParam = %q, want %q", got, "01AR3Z00000000000000000000")
	}
}

func TestPathParam_ExtractsNackJobID(t *testing.T) {
	got := pathParam("/jobs/01AR3Z00000000000000000000/nack", "/jobs/", "/nack")
	if got != "01AR3Z00000000000000000000" {
		t.Errorf("pathParam = %q, want %q", got, "01AR3Z00000000000000000000")
	}
}

func TestPathParam_ExtractsQueueFromNext(t *testing.T) {
	got := pathParam("/queues/examplequeue/next", "/queues/", "/next")
	if got != "examplequeue" {
		t.Errorf("pathParam = %q, want %q", got, "examplequeue")
	}
}

func TestPathParam_ExtractsWorkerID(t *testing.T) {
	got := pathParam("/workers/worker-abc-123", "/workers/", "")
	if got != "worker-abc-123" {
		t.Errorf("pathParam = %q, want %q", got, "worker-abc-123")
	}
}

func TestPathParam_OnlyPrefix(t *testing.T) {
	got := pathParam("/workers/", "/workers/", "")
	if got != "" {
		t.Errorf("pathParam = %q, want %q (no value after prefix)", got, "")
	}
}

func TestPathParam_NoMatchEmptyResult(t *testing.T) {
	got := pathParam("/", "/workers/", "")
	if got != "" {
		t.Errorf("pathParam = %q, want %q", got, "")
	}
}
